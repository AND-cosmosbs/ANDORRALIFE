        ...
        with cols[2]:
            if st.button(f"➕ Añadir {row['Referencia']}", key=row['Referencia']):
                st.session_state.carrito.append({
                    "ref": row["Referencia"],
                    "nombre": row["Nombre"],
                    "precio": float(row["PVP1"]) if pd.notnull(row["PVP1"]) else 0.0,
                    "url": row["URL Foto"]
                })
...
if st.session_state.carrito:
    ...
    if st.button("📄 Generar albarán/factura simple"):
        ...
        # 👇 Asegurar que el cliente_id es int y total es float
        pedido_data = {
            "id": pedido_id,
        ...
        if response_pedido.status_code != 201:
            ...
            st.text("📦 Datos enviados a Supabase:")
            st.json(pedido_data)
            st.text("📨 Respuesta de Supabase:")
            st.text(response_pedido.text)
        else:
            for item in st.session_state.carrito:
                linea = {
                    "pedido_id": pedido_id,
